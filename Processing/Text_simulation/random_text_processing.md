random\_text\_processing
================
Nicholas A. Lester

## Summary

This file processes the variation set count-level data from the randomly
simulated texts.
...
**Rank v. Frequency (relative) plots** First, we get the words.
...
Next, we compute frequencies and ranks for nouns and verbs (pooled
together).
...
Now we can plot (we also compute entropies overall to get an idea about
the relative skewedness of the distributions).
...
![](random_text_processing_files/figure-gfm/plotting_freq_v_rank-1.png)<!-- -->
...
![](random_text_processing_files/figure-gfm/plotting_freq_v_rank-2.png)<!-- -->
For both languages, CSS shows higher normalized entropy than ADS (i.e.,
it is richer in lexical diversity, word for word). The rank graphs show
that for the top 50 most frequent nouns and verbs, CSS frequncies
decline less rapidly, particularly for the most frequent words.

TODO: Get the by-session CSS lexical entropies and plot against VS
percentage for each language.

Here we ask to what extent the lexical entropy of the CSS in any given
session affects the VS percentage (note that this measure is computed in
the same manner as our measure of development; however, we now apply it
to the adult-produced CSS rather than the child’s own speech).
